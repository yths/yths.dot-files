...
if __name__ == "__main__":
    try:
        ...
        location_data = requests.get(
            f"https://ipinfo.io/{ip_address}?token={ipinfo_token}"
        ).json()
        lat, lng = location_data["loc"].split(",")
        tzid = location_data["timezone"]
        sun_data = requests.get(
            f"https://api.sunrise-sunset.org/json?lat={lat}&lng={lng}&tzid={tzid}&formatted=0"
        ).json()
        ...
        point = (
            influxdb_client.Point("location")
