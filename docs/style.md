...
Three files sit at the repository root rather than in `docs/`, because they are read before anything else: `setup.toml` is the one a reader edits, and `README.md` orients someone deciding whether to install this, and `CONTRIBUTING.md` tells someone changing it how to get a commit past the gate. Both stay short enough to read in full.
...
`docs/preview/` holds the rendered preview the README shows — generated by `helper/render_preview.py` from the theme bundle, and tracked rather than gitignored because GitHub has to serve it. `gendocs.py` fails if it no longer matches the palette, which is what keeps tracking a generated file honest.
