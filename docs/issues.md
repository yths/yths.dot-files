...
- [ ] Extend `helper/gendocs.py` with an import scanner that diffs the actual imports against `docs/dependencies.md`
    - (2026-08-30) unblocked. The scanner has to resolve the names in that page to compare them with real imports, and it could not while the "Used by" column mixed bare module names, bare filenames and paths rooted at three different directories. Every entry is now repo-rooted, and `gendocs.py` fails on any that stops resolving, so the remaining work is the diff itself.
