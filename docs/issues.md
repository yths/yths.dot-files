...
- [x] Integrate wallpaper generation script
    - (2026-08-30) integrated into `yths.themes` rather than here, which is where the palette already is. A generator downstream of the export would have to be handed the colours, the sizes and the four filenames — which is exactly what the `wallpaper-handoff.json` it used to write instead of images was.
    - (2026-08-30) it is a registry rather than a script: one module per renderer under `infrastructure/wallpapers/`, named in a table that implements nothing, the same shape the patchers here and the jobs in `yths.backend-service` use. `solid` is the first and the one the tracked bundle uses — no network, no key, no model, so `assets/default/` stays reproducible by anyone who clones this.
    - (2026-08-30) the bundle now carries `wallpapers.json`: which renderer drew the four images and at what size. A record of what happened, where the handoff was a brief for something that had not.
...
- [x] Have `yths.themes` export to `assets/<name>/` rather than `assets/theme-<uuid>/`
    ...
    - (2026-08-30) done on the generating side too. `yths.themes` resolves an export target before it assembles anything, and writes the bundle's directory name into the manifest's `name` as well — so the two cannot disagree and `install.py --theme <name>` works from a file listing. `ExportMode` there is the two-way switch this ticket implied but never said: `default` rewrites `assets/default/`, and `personal` writes `assets/<slug>/`.
    - (2026-08-30) `assets/*` with `!assets/default/` is now in `.gitignore` here, which is what makes a personal bundle usable without dirtying the tree. It is the half of the contract that lives in this repository: the generator can only put a directory there, it cannot decide whether the directory is committed.
- [x] Rename the `stream` Redis stream to `broadcast` in `yths.backend-service`
    ...
    - (2026-08-30) done on the publishing side too: `jobs/broadcast.py` in `yths.backend-service` publishes under the new name, so the fallback is now the only thing keeping an older backend readable rather than the only thing making the widget work. Deleting `LEGACY_STREAM_NAMES` is a separate change, deliberately not made here — it is safe once every machine runs a backend from 2026-08-30 or later, and there is no way for this repository to know that.
    - (2026-08-30) nothing removes the old key when a job stops writing it, so `redis-cli -n 1 DEL stream` clears the stale one on each machine after the backend is restarted.
- [x] Stop `yths.themes` emitting `monitors` and the stale `state` vocabulary in the bundle manifest
    ...
    - (2026-08-30) the generator followed. Its manifest writer now emits exactly `schema_version`, `name`, `font`, `wallpapers` and `state` — no `monitors`, no legacy `colors`, and `state.theme_mode` rather than `state.mode`. Tests there assert each of those absences by name, because every one of them was a key that rotted quietly rather than failing.
    - (2026-08-30) the third stale key went with them: the manifest carried a `colors` block in the vocabulary this repository dropped, which `install.py` was stripping defensively on the way in. It is no longer written.

- [x] `yths.themes` writes a `palette.pkl` this repository cannot read
    - (2026-08-30) the "partial only" note under the `nuunamnir` ticket, promoted to its own entry because it was the reason no generated bundle worked. The pickle held chromalytica's own `Palette` object: unpickling it needs that library, which no desktop has, and even with it the object does not subscript by token, so `configuration["palette"]["dark"]["red"]` could never have resolved.
    - (2026-08-30) the two artifacts are now named for what they are. `palette-source.pkl` is chromalytica's grid, a build artifact that never leaves the workspace; `palette.pkl` is `{mode: {token: hex}}`, written from the theme's editable palette at assembly, and the exporter excludes the source explicitly rather than by omission.
    - (2026-08-30) the vocabulary was wrong too, and separately: 30 tokens in the `nuunamnir` shape (`positive`, `pastel_*`, `effect_*`) against the 21 this repository's patchers index. `patch_kitty` alone would have raised `KeyError` on `red_variant`. The enum there is now this repository's vocabulary, and a contract test in `yths.themes` reads the tracked `assets/default/palette.pkl` and fails if the two ever diverge again.
