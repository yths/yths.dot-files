...
alias yay-update-all='export TMPFILE="$(mktemp)"; \
  sudo true; \
  rate-mirrors --entry-country=<CC> --save=$TMPFILE arch --max-delay=21600 \
...
Replace `<CC>` with your ISO country code (e.g. `DE`, `US`).
...
```bash
plymouthd --debug-file=$HOME/plymouth-test.log
plymouth --show-splash --debug
sleep 15
plymouth --quit
```

Logs land in `$HOME/plymouth-test.log` for inspection afterwards.
