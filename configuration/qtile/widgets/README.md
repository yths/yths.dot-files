...
helper belongs here, next to its callers. Symlinks to scripts elsewhere in the repo do not.
...
- **Everything else** — the load-bearing symlinks to scripts in `helper/` and JSON in `configuration/vscode/`, plus the standalone `test_audio.py` harness — should not live in this directory. See `docs/issues.md`.
